...
// ============================================================================
// MANUAL VENDOR — derived from JetBrains/compose-multiplatform-core
//   navigation3/navigation3-ui/src/commonMain/kotlin/androidx/navigation3/ui/NavDisplay.kt
//   (ref: tools/compose-fork/compose-ref.txt). NON-IDEMPOTENT — excluded from the
//   folder sync via a `!` line in compose-fork.txt; reconcile by hand on ref bumps.
// CHANGE vs upstream: the two `rememberLifecycleOwner(maxLifecycle=…)` calls are replaced
//   with `LocalLifecycleOwner.current`. rememberLifecycleOwner (lifecycle-runtime-compose)
//   launches an effect that spins without suspending on Kotlin/Native, stalling the frame
//   loop before first paint (works on JVM). Reusing the ambient owner keeps entries RESUMED;
//   we forgo the transient STARTED cap during transitions, which nothing here depends on.
// ============================================================================
...
public fun <T : Any> NavDisplay(
    ...
    transition.AnimatedContent(
        ...
        // MANUAL VENDOR (see file header): upstream used rememberLifecycleOwner(maxLifecycle=…)
        // to cap the scene lifecycle to STARTED mid-transition. That spins forever on
        // Kotlin/Native (lifecycle-runtime-compose), so reuse the ambient owner — entries stay
        // RESUMED (we don't rely on the STARTED-during-animation cap).
        val sceneLifecycleOwner = LocalLifecycleOwner.current
    ...
    currentOverlayScenes.fastForEachReversed { overlayScene ->
        key(overlayScene) {
            val overlaySceneLifecycleOwner = LocalLifecycleOwner.current  // MANUAL VENDOR: see header (K/N rememberLifecycleOwner spin)
