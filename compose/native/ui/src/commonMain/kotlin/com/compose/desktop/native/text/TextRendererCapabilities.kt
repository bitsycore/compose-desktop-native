...
/* No upstream equivalent — lives in com.compose.desktop.native.text per
   ...
   Material Symbols install() path, which emits a one-shot warning when
   variable-font axes won't be honoured. The Skia renderer sets
   supportsFontVariations = true via Typeface.makeClone; the SDL3 renderer
   leaves it false because SDL3_ttf 3.2 has no axis-set API (filename /
   iostream / size / face / dpi only).
