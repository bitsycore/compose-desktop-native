// compose-desktop-material-symbols:outlined — bundles the Material Symbols
// Outlined variable font and exposes a one-call install() that registers it
// with IconFont under the family name "material-symbols-outlined". The
// codepoint constants in core (MaterialSymbols) resolve identically against
// all three styles, so picking a style is just a matter of which module you
// depend on.
//
// The font (Apache 2.0, Google Fonts) is downloaded into build/iconFont/
// at build time and stitched into the consumer app's data.kres by the
// demo's Zip task — which auto-discovers every :compose-desktop-material-
// symbols:* module and pulls its "iconFontFile" task output. No Compose
// resources runtime is involved (we have our own .kres pipeline), so this
// module deliberately doesn't apply the compose-multiplatform plugin.
